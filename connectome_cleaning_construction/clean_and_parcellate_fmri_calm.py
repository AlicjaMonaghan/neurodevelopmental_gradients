...
def clean_and_parcellate_fmri_calm(dataset, participant_id):
    ...
    # set the path to the files depending on input data set, and the number of dummy volumes to remove.
    ...
    confound_file = pd.read_csv(
        func_path + participant_id + '/' + participant_id + '_task-rest_desc-confounds_timeseries.tsv',
        delimiter='\t').loc[dummy_tr:, ]
    bold_img = \
        nimg.load_img(func_path + participant_id + '/' + participant_id +
                      '_task-rest_space-MNI152NLin6Asym_desc-smoothAROMAnonaggr_bold.nii.gz').slicer[:, :, :, dummy_tr:]
    ...
    for parcellation in parcellation_list:
        ...
        # extract the lower triangle of the array with an offset of 1, apply fisher's z-transformation to edge weights,
    ...
    save_file_name = func_path + participant_id + '_cleaned_functional_connectomes.h5'
