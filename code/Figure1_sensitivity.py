...
########################################################
# Parsing dataset and neurotypicality effects in manifolds
########################################################
# Load the diffusion-map embedding outputs for the neurotypical portion of CALM
neurotypical_calm_dme = sio.loadmat('data/calm/dme/non-referred_schaefer200x7_baseline.mat')
# Calculate the mean of columns with the same name (Yeo network label)
neurotypical_calm_dme_sc = pd.DataFrame(
    neurotypical_calm_dme['structural_manifold_eccentricity'], columns=schaefer200_yeo7_labels).groupby(
    by=schaefer200_yeo7_labels, axis=1).mean()
neurotypical_calm_dme_sc['global'] = neurotypical_calm_dme_sc.mean(axis=1)
neurotypical_calm_dme_sc['BIDS'] = [x.strip(' ') for x in neurotypical_calm_dme['structural_sub_list']]
neurotypical_calm_dme_sc = neurotypical_calm_dme_sc.merge(
    calm_sc_master[calm_sc_master.Referred==0][['BIDS', 'scan_age', 'Sex', 'meanFWD', 'timepoint']], how='left')
neurotypical_calm_dme_fc = pd.DataFrame(
    neurotypical_calm_dme['functional_manifold_eccentricity'], columns=schaefer200_yeo7_labels).groupby(
    by=schaefer200_yeo7_labels, axis=1).mean()
neurotypical_calm_dme_fc['global'] = neurotypical_calm_dme_fc.mean(axis=1)
neurotypical_calm_dme_fc['BIDS'] = [x.strip(' ') for x in neurotypical_calm_dme['functional_sub_list']]
neurotypical_calm_dme_fc = neurotypical_calm_dme_fc.merge(
    calm_fc_master[calm_fc_master.Referred==0][['BIDS', 'scan_age', 'Sex', 'meanFWD', 'timepoint']], how='left')
# Load the network-level SC and FC manifold eccentricity for the referred subset of CALM, and add demographic data
referred_calm_sc_eccentricity = pd.read_csv('data/calm/dme/referred_calm_network_sc_eccentricity_df.csv').join(
    referred_calm_sc_dme[['sex', 'scan_age', 'meanfwd', 'timepoint']])
referred_calm_fc_eccentricity = pd.read_csv('data/calm/dme/referred_calm_network_fc_eccentricity_df.csv').join(
    referred_calm_fc_dme[['sex', 'scan_age', 'meanfwd', 'timepoint']])
# For each network division, construct a GLM where we predict mean eccentricity using neurotypicality (2 factors:
# neurotypical or neurodiverse), sex, meanFWD, and scan age. Do this for the baseline participants only, as CALM
# controls were only evaluated at baseline.
network_divisions = list(unique_schaefer200_yeo7_labels) + ['global']
for modality_idx, modality in enumerate(modalities):
    # Get the modality-specific DME data frames for CALM controls, referred CALM and NKI
    referred_calm_eccentricity = globals()[f'referred_calm_{modality}_eccentricity']
    control_calm_eccentricity = globals()[f'neurotypical_calm_dme_{modality}']
    # For each data frame, subset by the baseline session, and format the column names in the same way! Add two extra
    # columns to indicate neurotypicality and site.
    baseline_referred_calm_eccentricity = referred_calm_eccentricity[
        referred_calm_eccentricity.timepoint == "baseline"].assign(neurotypicality='referred')
    baseline_control_calm_eccentricity = control_calm_eccentricity[control_calm_eccentricity.timepoint == 0].rename(
        columns={'Sex': 'sex', 'meanFWD': 'meanfwd'}).assign(neurotypicality='control')
    baseline_control_calm_eccentricity['sex'] = baseline_control_calm_eccentricity['sex'].replace({0: 'M', 1:'F'})
    # Merge all three data frames together!
    retain_columns = network_divisions + (['sex', 'meanfwd', 'scan_age', 'neurotypicality'])
    parsing_df = pd.concat([baseline_referred_calm_eccentricity[retain_columns],
                            baseline_control_calm_eccentricity[retain_columns]])
    parsing_df.to_csv('data/calm/manifold_eccentricity_baseline_CALM_sc.csv', index=None)
    # Melt the data frame!
    parsing_df_melt = pd.melt(parsing_df, id_vars=['sex', 'meanfwd', 'scan_age', 'neurotypicality', 'participant'],
                              value_vars=list(unique_schaefer200_yeo7_labels), var_name='network', value_name='eccentricity')
    model = smf.mixedlm(formula="eccentricity ~ neurotypicality*network + scan_age + sex + meanfwd",
                        data=parsing_df_melt,  groups=parsing_df_melt["participant"], re_formula=None)
    # Rename global-level manifold eccentricity so that we can include it in our loop below
    parsing_df = parsing_df.rename(columns={'global': 'global_manifold'})
    network_divisions_updated = ['global_manifold' if x == 'global' else x for x in network_divisions]
    # Standardise all numeric data types so that we can extract standardised beta coefficients
    numeric_cols = parsing_df.select_dtypes(include=[np.number]).columns
    parsing_df[numeric_cols] = parsing_df[numeric_cols].apply(zscore)
    # Report how many neurotypical and neurodivergent participants we have
    neurotypical_nsub = parsing_df.neurotypicality.value_counts()['control']
    neurodivergent_nsub = parsing_df.neurotypicality.value_counts()['referred']
    print(f'{neurotypical_nsub} neurotypical and {neurodivergent_nsub} neurodivergent participants in {modality}')
    # For each network division, conduct a GLM where we predict mean network-level manifold eccentricity using scan age,
    # head motion, sex, and neurotypicality. Start by initialising an array to hold the modality-specific GLM
    # coefficient, standard error, and p-values (both raw and FDR-corrected) for the neurotypicality effect.
    modality_neurotypicality_effect = np.zeros((len(network_divisions_updated), 5))
    for network_idx, network in enumerate(network_divisions_updated):
        formula = f'{network} ~ neurotypicality + meanfwd + scan_age + sex'
        parsing_GLM = smf.glm(formula=formula, data=parsing_df).fit(cov_type='HC0')
        parsing_GLM_summary = extract_glm_summary(parsing_GLM)
        # Extract the row corresponding to neurotypicality
        neurotypicality_effect = parsing_GLM_summary[parsing_GLM_summary.index.str.contains('neurotypicality')]
        # Extract the standardised beta coefficient, standard error, and raw p-value: a positive beta coefficient
        # suggests that manifold eccentricity in the neurodivergent children is higher than in neurotypical children
        modality_neurotypicality_effect[network_idx, 0:3] = neurotypicality_effect[['estimates', 'std_err', 'raw_pval']]
        # And in Cohen's D as an effect size indicator
        neurotypicality_t_value = parsing_GLM.tvalues[parsing_GLM.tvalues.index.str.contains('neurotypicality')].values
        modality_neurotypicality_effect[network_idx, 3] = compute_effsize_from_t(
            neurotypicality_t_value.item(), neurotypical_nsub, neurodivergent_nsub)
    # Correct for multiple comparisons!
    modality_neurotypicality_effect[:, 4] = false_discovery_control(modality_neurotypicality_effect[:, 2])
    # Format as a data frame and save!
    modality_neurotypicality_effect_df = pd.DataFrame(
        modality_neurotypicality_effect, index=network_divisions_updated,
        columns=['estimates', 'std_err', 'raw_pval', 'd', 'fdr_pval'])
    modality_neurotypicality_effect_df.to_csv(f'data/{modality}_neurotypicality_effect_eccentricity.csv')
